...
public class Benchmark {
    ...
    private static int totalSPCalls = 10_000_000_00;
    ...
    public Benchmark(Map<String, List<String>> args) {
        ...
        this.m_clientCon = Benchmark.getClient(this.servers, 0);
        this.simulator = new RetwisSimulation(this.m_clientCon, this.async);
    }

    private static ClientConnection getClient(String servers, int id) {
        int sleep = 1000;
        while(true) {
            try {
    ...
    class SingleClientRunnable implements Runnable {
        ...
        SingleClientRunnable(int id, int totalSPCalls, String servers, boolean async) {
            ...
            ClientConnection client = Benchmark.getClient(servers, id);
            this.sim = new RetwisSimulation(client, async);
            this.sim.set_next_ids(51200, 8192);
        }

        public void run() {
            RetwisCallback callback = new RetwisCallback(false);
            for (int i = 0; i < this.totalSPCalls; i++) {
                try {
                    //
                    this.sim.doGetPosts(callback);
